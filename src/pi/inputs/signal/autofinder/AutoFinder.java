...
public class AutoFinder {
	...
	public void autoFind() {
		...
		this.simplifySignal(!this.params.DOWN_SIGNAL,
				this.params.SIMPLIFIED_RANGE, this.signal);
		...
		for (int i = 0; i < size; i++) {
			...
			if (range != null) {
				Range leftRange = this.getMonotonic(i - 1, false, true,
						this.params.QRS_ASC_DERIV, this.params.QRS_MAX_NEG_ASC,
						timeAsc, false);

				Range rightRange = this.getMonotonic(range.getRight(), true,
						true, this.params.QRS_RIGHT_UP_DERIV,
						this.params.QRS_MAX_NEG_RIGHT_UP, timeRightUp, false);
		...
		while (mItr.hasNext()) {
			...
			if (this.params.T_IS_UP) {
				resizeRight = this.getMonotonic(maxPointer + 1, true, false,
						this.params.T_RIGHT_DERIV, this.params.T_RIGHT_NEG,
						time, false);
